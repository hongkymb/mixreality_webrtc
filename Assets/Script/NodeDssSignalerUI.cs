...
public class NodeDssSignalerUI : MonoBehaviour
{
    ...
    private void Start()
    {
        const string PEER_ID_KEY = "PEER_ID";
        if (NodeDssSignaler.LocalPeerId == null || NodeDssSignaler.LocalPeerId == "")
        {
            string id = GenerateRandomNo().ToString();


            NodeDssSignaler.LocalPeerId =   id;
        }
        string localPeerId = "PeerID: " + NodeDssSignaler.LocalPeerId;
